...
ENV VIRTUAL_ENV /opt/oid-pocketbase-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/oid-pocketbase/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/oid-pocketbase/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/oid_pocketbase/ && touch src/oid_pocketbase/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/oid-pocketbase/
